...
class Shortcut_att(nn.Module):
    def __init__(self, ouc, dimension=0):
...
class SPPCSPC_sa(nn.Module):
    # CSP https://github.com/WongKinYiu/CrossStagePartialNetworks
    def __init__(self, c1, c2, n=1, shortcut=False, g=1, e=0.5, k=(5, 9, 13)):
        super(SPPCSPC_sa, self).__init__()
        c_ = int(2 * c2 * e)  # hidden channels
        self.cv1 = Conv(c1, c_, 1, 1)
        self.cv2 = Conv(c1, c_, 1, 1)
        self.cv3 = Conv(c_, c_, 3, 1)
    ...
    def forward(self, x):
        ...
        y1 = self.cv6(self.cv5(torch.cat([x1] + [m(x1) for m in self.m], 1)))
        y2 = self.cv2(x)
        y_out = self.att(self.cv7(torch.cat((y1, y2), dim=1)))
        avg_out = torch.mean(y_out, dim=1, keepdim=True)
        max_out, _ = torch.max(y_out, dim=1, keepdim=True)  # 30,1,50,30
        att = torch.cat([avg_out, max_out], dim=1)
        att = self.conv_sp(att)  # 30,1,50,30
...
class Yolov7x_E_ELAN(nn.Module):
    ...
    def forward(self, x):
        ...
        return x_final
class Yolov7_E_ELAN_R(nn.Module):
    def __init__(self, inc, ouc, hidc, act=True):
        super(Yolov7_E_ELAN_R, self).__init__()

        self.conv1 = Conv(inc, hidc, k=1, act=act)

        self.Res1 = Res(hidc // 2, hidc // 2)

        self.Res2 = Res(hidc // 4, hidc // 4)

        self.Res3 = Res(hidc // 8, hidc // 8)

        self.conv7 = Conv(hidc, ouc, k=1, act=act)

    def forward(self, x):
        x1 = self.conv1(x)

        num_channels = x1.size(1)
        x2 = x1[:, :num_channels // 2, :, :]
        x2_res = x1[:, num_channels // 2:num_channels, :, :]
        x3 = self.Res1(x2)

        # x4 = self.half_channels2(x3)
        x4 = x3[:, :num_channels // 4, :, :]
        x4_res = x3[:, num_channels // 4:num_channels // 2, :, :]
        x5 = self.Res2(x4)

        # x6 = self.half_channels3(x5)
        x6 = x5[:, :num_channels // 8, :, :]
        x6_res = x5[:, num_channels // 8:num_channels // 4, :, :]
        x7 = self.Res3(x6)

        x_concat = torch.cat([x2_res, x4_res, x6_res, x7], dim=1)

        x_final = self.conv7(x_concat)
        return x_final
class Yolov7_E_ELAN_R12(nn.Module):
    def __init__(self, inc, ouc, hidc=256, act=True):
        super(Yolov7_E_ELAN_R12, self).__init__()

        self.conv1 = Conv(inc, hidc, k=1, act=act)

        self.Res1 = Res(hidc // 4, hidc // 4)

        self.Res2 = Res(hidc // 4, hidc // 4)

        self.Res3 = Res(hidc // 4, hidc // 4)

        self.Res4 = Res(hidc // 4, hidc // 4)
    ...
    def forward(self, x):
        ...
        num_channels = x0.size(1)
        x1 = x0[:, :num_channels // 4, :, :]
        x2 = x0[:, num_channels // 4:num_channels // 2, :, :]
        x3 = x0[:, num_channels // 2:num_channels // 4*3, :, :]
        x4 = x0[:, num_channels // 4*3:num_channels,:,:]
        x1_res = self.Res1(x1)
        x2_in = x2 + x1_res
        x2_res = self.Res2(x2_in)
        x3_in = x3 + x2_res
        x3_res = self.Res3(x3_in)
        x4_in = x4 + x3_res
        x4_res = self.Res4(x4_in)

        x_concat = torch.cat([x1_res,x2_res,x3_res, x4_res], dim=1)


        x_final = self.conv7(x_concat)
        return x_final
    
class Yolov7_E_ELAN_RR(nn.Module):
    def __init__(self, inc, ouc, hidc, act=True):
        super(Yolov7_E_ELAN_RR, self).__init__()

        self.resConv = Conv(inc, ouc,k=3,act=act)

        self.conv1 = Conv(inc, hidc, k=1, act=act)

        self.Res0 = Res(ouc,ouc)
...
class RC2F(nn.Module):
    ...
    def forward(self, x):
        ...
        x1 = self.conv1(x)
        x1_res = self.resConv(x)
        x1_l = x1[:, :num_channels // 2, :, :]
        x1_r = x1[:, num_channels // 2:num_channels, :, :]

        x2 = self.Res1(x1_l+x1_r)
        x2_l = x2[:, :num_channels // 4, :, :]
        x2_r = x2[:, num_channels // 4:num_channels // 2, :, :]

        x3 = self.Res2(x2_l+x2_r)
        x3_l = x3[:, :num_channels // 8, :, :]
        x3_r = x3[:, num_channels // 8:num_channels // 4, :, :]

        x4 = self.Res3(x3_r+x3_l)

        x_concat = torch.cat([x1_l, x2_l, x3_l, x4], dim=1)

        x_11 = self.conv7(x_concat)

        return x1_res + x_11
    
class Yolov7_E_ELAN_RR_m(nn.Module):
    def __init__(self, inc, ouc, hidc, act=True):
        super(Yolov7_E_ELAN_RR_m, self).__init__()
...
class Yolov7_E_ELAN_RR_s(nn.Module):
    def __init__(self, inc, ouc, hidc, act=True):
        super(Yolov7_E_ELAN_RR, self).__init__()
...
class Yolov7_E_ELAN_RR_3(nn.Module):
    def __init__(self, inc, ouc, hidc, act=True):
        super(Yolov7_E_ELAN_RR_3, self).__init__()
        self.ELANRR1 = Yolov7_E_ELAN_RR(inc,inc,hidc)
        self.ELANRR2 = Yolov7_E_ELAN_RR(inc,inc,hidc)
        self.ELANRR3 = Yolov7_E_ELAN_RR(inc,ouc,hidc)
...
# class V7DownSampling_Neck_shortcut(nn.Module):
#     def __init__(self, inc, ouc) -> None:
#         super(V7DownSampling_Neck_shortcut, self).__init__()
#
#         self.maxpool = nn.Sequential(
#             nn.MaxPool2d(kernel_size=2, stride=2),
#             Conv(inc, inc, k=1)
...
#         self.conv = nn.Sequential(
#             Conv(inc, inc, k=1),
#             Conv(inc, inc, k=3, s=2),
#         )
#         self.conv2 = Conv(ouc,2*ouc,k=1,s=1)
#
#     def forward(self, x):
#           y = torch.cat([self.maxpool(x[0]), self.conv(x[0])], dim=1)
#           print(x[1].shape)
#           final = y+x[1]
#           return self.conv2(final)
...
class V7DownSampling_Neck_shortcut(nn.Module):
    def __init__(self, inc, ouc) -> None:
        ...
        self.conv2 = Conv(2*inc,4*ouc,k=1,s=1)

    def forward(self, idx):
          x = torch.cat([self.maxpool(idx[0]), self.conv(idx[0])], dim=1)
          x = x+idx[1]
          return self.conv2(x)
...
class V7DownSampling_Neck_shortcut_att(nn.Module):
    def __init__(self, inc, ouc) -> None:
        super(V7DownSampling_Neck_shortcut_att, self).__init__()
...
#     def forward(self, x):
#         x = self.ups(x)
#         return self.conv2(self.m(self.conv1(x)))
class fusion2(nn.Module):
    def __init__(self, c1, c2):
        super(fusion2, self).__init__()
        self.ups = nn.PixelShuffle(2)
    ...
    def forward(self, x):
        x = self.ups(x)
        x = self.conv2(self.m(self.conv1(x)))
        return x
...
class fusion3(nn.Module):
    ...
    def forward(self, x):
        x1 = self.ups(x)
        x2 = self.conv2(self.m(self.conv1(x1)))
        x3 = self.conv3(x1)
